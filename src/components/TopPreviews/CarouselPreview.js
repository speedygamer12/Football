...
const CarouselPreview = () => {
  return (
    <div className='md:py-5 pt-5 pb-5 md:h-[580px] flex flex-col md:w-2/3 w-full' >
        <Carousel autoPlay={true} infiniteLoop={true} showArrows={false} showStatus={false} showThumbs={false} >
            {images?.map((image, index) => {
                return <img key={`${index} ${image}`} src={image} alt='Advert' className='object-cover w-full md:h-[457px]' />
            })}
        </Carousel>
        <div className='p-5 bg-[#F9FBFC] h-[123px] rounded-b-[4px] border-b border-x border-[#EAECED]' >
            <p className='text-[#000A0F] font-semibold text-[16px] leading-[24px] md:text-[24px] md:leading-[36px] ' >Manchester United Vs Chelsea: Battle of the mid</p>
