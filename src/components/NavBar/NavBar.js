...
const NavBar = () => {
    const navTabs = ["Premier League", "Championship ", "League One", "LeagueTwo", "HomeOutside"]
    const [ openNav, setOpenNav ] = useState(false)
  return (
    <>
        <nav className={` ${openNav && " md:flex"} sticky z-[1] top-0 flex bg-[#FDFDFD] w-full h-[68px] md:h-[120px] p-5 md:px-10 md:py-7 items-center justify-between`} >
            <div className='' >
                <Link to="/" >
        ...
        <nav className={`bg-[#FDFDFD] w-full h-screen fixed top-0 left-0 z-[2] p-5 ${openNav ? "block" : "hidden"} md:hidden`} >
            ...
            <ul className="flex md:flex-row flex-col list-none">
                {navTabs?.map(item => <li className='text-[18px] text-[#000A0F] font-medium leading-[27px] my-3' key={item} ><Link to="/league" >{item}</Link></li>)}
            </ul>
